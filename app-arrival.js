...
function setDatetime(){ $("datetime").value = `${todayISO()} ${nowTime()}`; }
function fillBases(){
  $("base").innerHTML = `<option value="">選択</option>` + BASES.map(b=>`<option>${b}</option>`).join("");
}
function calcOdo(){
  const a = Number(($("odoStart").value||"").trim());
  const b = Number(($("odoEnd").value||"").trim());
  $("odoDiff").value = (!Number.isNaN(a)&&!Number.isNaN(b)&&a>0&&b>=a) ? String(b-a) : "";
}
...
const INSPECTION_ITEMS = [
  ["tirePressure","タイヤ空気圧"],["tireTread","タイヤ溝/ひび割れ"],["wheelNut","ホイールナット緩み"],
  ["brake","ブレーキ効き"],["parkingBrake","パーキングブレーキ"],["steering","ハンドル操作"],
  ["lights","ライト（前照/尾灯/ブレーキ/ウインカー/ハザード）"],["wiperWasher","ワイパー/ウォッシャー液"],
  ["mirrorGlass","ミラー/ガラス破損"],
  ["engineOil","エンジンオイル量"],["coolant","冷却水"],["battery","バッテリー（警告灯含む）"],
  ["noiseSmellVibration","異音/異臭/異常振動"],["leak","漏れ（オイル/冷却水）"],["damage","外装破損"],
  ["loadState","積載状態（偏り/過積載なし）"],
  ["extinguisher","消火器"],["triangle","三角停止板"],["reflectVest","反射ベスト"],["tools","ジャッキ/工具（任意でもOK）"],
];
function renderInspection(){
  $("inspectionBox").innerHTML = INSPECTION_ITEMS.map(([key,label])=>{
...
function readInspection(){
  const obj = {}; let anyNg=false; let missingReq=false;
  INSPECTION_ITEMS.forEach(([key])=>{
...
async function uploadMany(files, prefix){
  ...
  for(const f of files){
    arr.push(await uploadOne(f, `${prefix}/${Date.now()}_${Math.random().toString(16).slice(2)}_${f.name}`));
  }
  return arr;
}
...
async function submit(){
  ...
  try{
    ...
    if(projs.some(p=>!p.name || !p.area)) throw new Error("稼働案件（案件名・エリア）は全て必須です");
    ...
    const docRef=await addDoc(collection(db,"tenko"),{
      ...
      driverName,base,vehicleNo,
      odoStart,odoEnd,odoDiff,
      method,

      temp,condition,fatigue,
      medicineFlag,
      medicineDetail: medicineFlag==="あり" ? medicineDetail : "",
      breakMinutes,

      drinkFlag,alcoholJudge,alcoholValue,
      licenseNo,
    ...
    if(insp.anyNg && !inspectionNgDetail){
      ...
      throw new Error("点検にNGがあるため、NG詳細メモは必須です（保存済み。追記して再送信してください）");
    }
    ...
    if(alcoholFile) updates.alcoholPhotoUrl=await uploadOne(alcoholFile,`${prefix}/alcohol/${alcoholFile.name}`);
    const licenseFile=$("licensePhoto").files?.[0];
    if(licenseFile) updates.licensePhotoUrl=await uploadOne(licenseFile,`${prefix}/license/${licenseFile.name}`);
    const abnormalFiles=Array.from($("abnormalPhotos").files||[]);
    if(abnormalFiles.length) updates.abnormalPhotoUrls=await uploadMany(abnormalFiles,`${prefix}/abnormal`);
    const ngPhotos=Array.from($("inspectionNgPhotos").files||[]);
    if(ngPhotos.length) updates.inspectionNgPhotoUrls=await uploadMany(ngPhotos,`${prefix}/inspectionNg`);
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  setDatetime(); fillBases(); renderProjects(); renderInspection(); calcOdo();
});
