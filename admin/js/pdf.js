...
function calcMonthlySummary(group, filters){
  ...
  // 日報の数値集計（任意項目なので入っているものだけ）
  let totalPay = 0;
  let totalCost = 0;
  let totalProfit = 0;

  group.daily.forEach(d=>{
    totalPay    += safeNum(d.salesTotal);
    totalCost   += safeNum(d.costTotal);
    totalProfit += safeNum(d.profit);
  });
...
function buildMonthlyHtml(group, filters){
  ...
  // テーブル行生成（点呼）
  const tenkoRows = group.tenko.map(t=>{
    const d = normalizeDate(t.at);
  ...
  // テーブル行生成（日報）
  const dailyRows = group.daily.map(d=>{
    return `
      <tr>
        <td class="td">${normalizeDate(d.date)}</td>
        <td class="td">${d.mainProject || ""}</td>
        <td class="td">${d.odoDiff ?? ""}</td>
        <td class="td">${d.salesTotal ?? ""}</td>
        <td class="td">${d.costTotal ?? ""}</td>
